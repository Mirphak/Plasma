#Decompiled with Drizzle31!  Enjoy :)
...
resetVal = ptAttribInt(6, 'Reset to this value when noone is there', default=0)
AgeStartedIn = None
OmmitNotify = False

class TLALoftAgeSDLBoolRespondReset(ptResponder):
    ...
    def OnServerInitComplete(self):
        ...
        if (AgeStartedIn == PtGetAgeName()):
            ...
            if ((type(stringVarName.value) == type('')) and (stringVarName.value != '')):
                ...
                if ((not (len(PtGetPlayerList()))) and (ageSDL[stringVarName.value][0] != resetVal.value)):
                    OmmitNotify = True
                    ageSDL[stringVarName.value] = (resetVal.value,)
                ageSDL.setNotify(self.key, stringVarName.value, 0.0)
                if ageSDL[stringVarName.value][0]:
                    PtDebugPrint(('DEBUG: xAgeSDLBoolRespond.OnServerInitComplete:\tRunning true responder on %s, fastforward=%d' % (self.sceneobject.getName(), boolFFOnInit.value)))
                    respBoolTrue.run(self.key, fastforward=boolFFOnInit.value)
                else:
                    PtDebugPrint(('DEBUG: xAgeSDLBoolRespond.OnServerInitComplete:\tRunning false responder on %s, fastforward=%d' % (self.sceneobject.getName(), boolFFOnInit.value)))
                    respBoolFalse.run(self.key, fastforward=boolFFOnInit.value)
            else:
                PtDebugPrint('ERROR: xAgeSDLBoolRespond.OnServerInitComplete():\tERROR: missing SDL var name')
