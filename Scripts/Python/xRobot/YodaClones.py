...
je crée un Dictionnaire (dicDemandeurs) avec une entrée par script (demandeur) je memorise CloneKey et nbClones demandés
...
def DechargerClones(masterKey):#     devalide  tout les clones de l objet   masterKey
    """
    !!! si un autre Magic a aussi crée des clones de meme type il peut  planter  à l'execution de DechargerClones et tout autre joueur dans certains cas
    """
...
#
#===================================================================
# Version actuelle Yodawave n'agis que sur les clones qu'il a créé =
#===================================================================
# Clonage d'un object

"""
    Gestion des clones  permet de generer mes clones sans influencer  les autres, 
    gere un dictionnaire des demandeurs et des clones leur appartenant.
    /!\ Si reload perte du Dictionnaire (dicDemandeurs) donc clones orphelins
    Le nom de demandeur doit etre valide
    Il y a un Sceneobject par objet ou par clone donc un cloneKey par clone
"""
...
#
def GetClones(nomDemandeur="", masterKey=None, nbClones=0):
    """
        Regarde si nomDemandeur, masterKey.getName existe dans dicDemandeurs 
        Renvoie la liste de CloneKey de nbClones demandée
        Sinon retourne une liste vide 
    """
    global dicDemandeurs
    if nomDemandeur in dicDemandeurs:
        try:
            if len(dicDemandeurs[nomDemandeur][masterKey.getName()]) >= nbClones:
                listeRetour = []
                CloneKeys = PtFindClones(masterKey)
                for CloneKey in dicDemandeurs[nomDemandeur][masterKey.getName()]:
                    if CloneKey in CloneKeys:               #/!\ Les clones sont-ils encore actifs ?
                        listeRetour.append(CloneKey)
                        if len (listeRetour) == nbClones:   # Ne retourne que le nombre de clones voulu
                            return listeRetour
                    else:
                        PtSendKIMessage(26, "GetClones CloneKey {} n'est plus valide".format(masterKey.getName()))
                        return []
            return dicDemandeurs[nomDemandeur][masterKey.getName()]
        except KeyError:
            PtSendKIMessage(26, "GetClones  KeyError {}".format(masterKey.getName()))
            return []
    else:
        return []
...
#
# liste des demandeurs valide
# version de Yodawave, que je ne dois donc pas utiliser
yActions = ("Marble", "Spiral", "Laser", "Gasper", "AttObjetSurAvatar", "PosClone", "CdMixologie", "Spark", "AnimationArche")
mActions = ("Pumpkin")

#
class InitClonage():
    """
        Gestion des clones permet d'avoir plusieurs types de clones par demandeur et 
        plusieurs demandeurs ayant le meme type de clones.
        Memorise les demandeurs, les types de clone (CloneKey) dans un dictionnaire 
        /!\ Reloader le module provoque des orphelins
        => Perte du Dic, delete est impossible reste desactivation activation.
    """
    #
    def __init__(self, nomDemandeur="", masterKey=None, nbClones=0):
        global dicDemandeurs
        self._dicDemandeurs = dicDemandeurs
    ...
    def onAlarm(self, param):
        ...
        if param == 2: # --  DemandeClone
            ...
            if self._nomDemandeur == "":
                ...
                return
            if not self._nomDemandeur in mActions: #("Marble", "Spiral", "Laser", "Gasper", "AttObjetSurAvatar", "PosClone", "CdMixologie", "Spark", "AnimationArche"):# liste des demandeurs valide
                PtSendKIMessage(26, "nomDemandeur Non valide")
                return
                
            if self._nomDemandeur in self._dicDemandeurs: # -- Est-ce que le demandeur est deja dans dicDemandeurs?
                if self._masterKey.getName() in self._dicDemandeurs[self._nomDemandeur]: # -- Est-ce que masterKey.getName est deja dans dicDemandeurs?
                    self._ListCloneKeyDemandeur = self._dicDemandeurs[self._nomDemandeur][self._masterKey.getName()]
                    if len(self._ListCloneKeyDemandeur) == self._nbClones: # -- Il existe et le nbr est suffisant: inventaire des clones et les recharger
                        PtSetAlarm(0.0, self, 5)
                    else:
                        if len(self._ListCloneKeyDemandeur) > self._nbClones: # -- nbr > a la demande: renvoyer et recharger le Nbr demandé (fait par GetClones)
                            PtSetAlarm(0.0, self, 5)
                        if len(self._ListCloneKeyDemandeur) < self._nbClones: # -- nbr < a la demande: cloner le nbr manquant, renvoyer et recharger le nbr demandé, OK
                            PtSetAlarm(0.0, self, 4)
                else: # -- Demandeur est deja dans dicDemandeurs, mais pas masterKey.getName
                    self._dicDemandeurs[self._nomDemandeur] [self._masterKey.getName()] = []
                    self._cloneKeysNonPerso = PtFindClones(self._masterKey)
                    PtSetAlarm(0.0, self, 4)
            else: # -- Inventaire inutile, ils ne m'appartiennent pas. Prendre en compte la commande
                self._dicDemandeurs[self._nomDemandeur] = {}
                self._dicDemandeurs[self._nomDemandeur][self._masterKey.getName()] = []
                self._cloneKeysNonPerso = PtFindClones(self._masterKey)
                PtSetAlarm(0.0, self, 4)
            return
        ...
        if param == 4:# -- Creation des clones 
            ...
            for i in range(self._nbClones - CloneObtenue):
                PtCloneKey(self._masterKey)
        ...
        if param == 5:#-------------------------------------------------------------------------------------------------------------------------------------  inventaire  des Clones
            ...
            for CloneKey in self._ListCloneKeyDemandeur:#                         on verifie qu"il sont toujours Actif
                if CloneKey in ListCloneKeyActif:
                    CloneActif +=1
            if CloneActif  ==  self._nbClones: #                                                ok  on  les recharge et retour
        ...
        if param == 6:#-------------------------------------------------------------------------------------------------------------------------------------  on recharge les clones et memorise la liste
            ...
            self._dicDemandeurs[self._nomDemandeur][self._masterKey.getName()] = self._ListCloneKeyDemandeur
            return


        if param == 7:#-------------------------------------------------------------------------------------------------------------------------------------  revalider  les clones d un demandeur          (Regenere)
            if self._nomDemandeur in self._dicDemandeurs:
                self._cloneKeys = self._dicDemandeurs[self._nomDemandeur][self._masterKey.getName()]
                for  CloneKey  in self._cloneKeys:
                    PtCloneKey(CloneKey, 1)
            return


        if param == 8:#-------------------------------------------------------------------------------------------------------------------------------------   devalider  les clones d un demandeur ne les suprime pas du    dicDemandeurs
            if self._nomDemandeur in self._dicDemandeurs:
                if self._masterKey.getName()  in self._dicDemandeurs[self._nomDemandeur]:
                    self._cloneKeys = self._dicDemandeurs[self._nomDemandeur][self._masterKey.getName()]
                    for  CloneKey  in self._cloneKeys:
                        PtCloneKey(CloneKey, 0)
            return
